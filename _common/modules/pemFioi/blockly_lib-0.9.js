...
var languageStrings = {
  fr: {
    ...
    importFromBlockly: "IMpoty from blockly",
    saveOrLoadProgram: "Save or load program:",
    avoidReloadingOtherTask:
      "Attention : ne rechargez pas le programme d'un autre sujet !",
    files: "Files",
    reloadProgram: "Reload :",
    saveProgram: "Save",
    limitBlocks: "{remainingBlocks} remaning blocs on {maxBlocks} allowed.",
    limitBlocksOver:
  ...
  return {
    ...
    mediaUrl:
      window.location.protocol == "file:" && modulesPath
        ...
        : "http://static3.castor-informatique.fr/contestAssets/blockly/",
    ...
    loadHtml: function (nbTestCases) {
      $("#blocklyLibContent").html(
        "<xml id='toolbox' style='display: none'></xml>" +
          ...
          this.strings.blocklyLanguage +
          "</option>" +
          "        <option value='javascript'>" +
          this.strings.javascriptLanguage +
          "</option>" +
          "      </select>" +
          "      <input type='button' class='language_javascript' value='" +
          this.strings.importFromBlockly +
      ...
      var saveOrLoadModal =
        "<div class='modal'>" +
        "    <p><b>" +
        this.strings.saveOrLoadProgram +
        "</b></p>\n" +
        "    <button type='button' class='btn' onclick='task.displayedSubTask.blocklyHelper.saveProgram()' >" +
        this.strings.saveProgram +
        "</button><span id='saveUrl'></span>\n" +
        "    <p>" +
        this.strings.avoidReloadingOtherTask +
        "</p>\n" +
        "    <p>" +
      ...
      if (nbTestCases > 1) {
        gridButtonsBefore +=
          ...
          this.strings.previousTestcase +
          "' onclick='task.displayedSubTask.changeTest(-1)'/>\n" +
          "  <span id='testCaseName'>Test 1</span>\n" +
          "  <input type='button' value='" +
          this.strings.nextTestcase +
          "' onclick='task.displayedSubTask.changeTest(1)'/>\n" +
          "</div>\n";
      }
      ...
      var gridButtonsAfter =
        "<div id='selectSpeed'>" +
        "  <div class='btn-group'>\n" +
        "    <button type='button' class='btn btn-default btn-icon' onclick='task.displayedSubTask.stop()'>" +
        this.strings.stopProgram +
        " </button>\n" +
        "    <button type='button' class='btn btn-default btn-icon' onclick='task.displayedSubTask.step()'>" +
        this.strings.stepProgram +
        " </button>\n" +
        "    <button type='button' class='btn btn-default btn-icon' onclick='task.displayedSubTask.changeSpeed(200)'>" +
        this.strings.slowSpeed +
        "</button>\n" +
        "    <button type='button' class='btn btn-default btn-icon' onclick='task.displayedSubTask.changeSpeed(50)'>" +
        this.strings.mediumSpeed +
        "</button>\n" +
        "    <button type='button' class='btn btn-default btn-icon' onclick='task.displayedSubTask.changeSpeed(5)'>" +
        this.strings.fastSpeed +
        "</button>\n" +
        "    <button type='button' class='btn btn-default btn-icon' onclick='task.displayedSubTask.changeSpeed(0)'>" +
        this.strings.ludicrousSpeed +
        "</button>\n" +
        "  </div>" +
        "</div>";
      gridButtonsAfter +=
        ...
        (this.scratchMode
          ? "<img src='" +
            this.mediaUrl +
            "icons/event_whenflagclicked.svg' height='32px' width='32px' style='vertical-align: middle;'>"
          : "") +
    ...
    load: function (language, display, nbTestCases, options) {
      ...
      if (display) {
        ...
        if (this.trashInToolbox) {
          Blockly.Trashcan.prototype.MARGIN_SIDE_ =
            $("#blocklyDiv").width() - 110;
        }
        ...
        function onchange(event) {
          ...
          var strLimitBlocks =
            remaining < 0
              ? that.strings.limitBlocksOver
              : that.strings.limitBlocks;
      ...
      this.programs = [];
      for (
        var iPlayer = this.mainContext.nbRobots - 1;
        iPlayer >= 0;
        iPlayer--
      ) {
        this.programs[iPlayer] = {
          blockly: null,
          blocklyJS: "",
          blocklyPython: "",
          javascript: "",
    ...
    completeBlockJson: function (block, objectName, categoryName, context) {
      ...
      if (
        typeof block.blocklyJson.output == "undefined" &&
        ...
        if (block.yieldsValue) {
          ...
          if (this.scratchMode) {
            ...
            block.blocklyJson.colourSecondary =
              Blockly.Colours.sensing.secondary;
      ...
      // Add parameters
      if (
        typeof block.blocklyJson.args0 == "undefined" &&
      ...
      if (typeof block.blocklyJson.message0 == "undefined") {
        ...
        if (typeof block.blocklyJson.message0 == "undefined") {
          block.blocklyJson.message0 =
            "<translation missing: " + block.name + ">";
        }
            ...
            blockInfo.codeGenerators[language] = function (block) {
              ...
              for (var iArgs0 in args0) {
                if (args0[iArgs0].type == "input_value") {
                  ...
                  params += Blockly[language].valueToCode(
                    block,
                    "PARAM_" + iParam,
    ...
    createSimpleGenerator: function (label, code, type, nbParams) {
      var jsDefinitions = this.definitions["javascript"]
        ...
        : [];
      var pyDefinitions = this.definitions["python"]
        ? this.definitions["python"]
        : [];
      ...
      Blockly.JavaScript[label] = function (block) {
        ...
        for (var iParam = 0; iParam < nbParams; iParam++) {
          ...
          params += Blockly.JavaScript.valueToCode(
            block,
            "NAME_" + (iParam + 1),
      ...
      Blockly.Python[label] = function (block) {
        ...
        for (var iParam = 0; iParam < nbParams; iParam++) {
          ...
          params += Blockly.Python.valueToCode(
            block,
            "NAME_" + (iParam + 1),
    ...
    createSimpleGeneratorsAndBlocks: function () {
      for (var genName in this.simpleGenerators) {
        for (
          var iGen = 0;
          iGen < this.simpleGenerators[genName].length;
          iGen++
        ) {
    ...
    getBlocklyLibCode: function (generators) {
      ...
      strCode +=
        "Math['max'] = function(a, b) { if (a > b) return a; return b; };\n";
      strCode +=
        "Math['min'] = function(a, b) { if (a > b) return b; return a; };\n";
      return strCode;
    },
    ...
    getStdBlocks: function () {
      ...
      for (var category in stdBlocks) {
        ...
        for (var b = 0; b < blockList.length; b++) {
          var blocklyName = blockList[b].depends
            ? blockList[b].depends
            : blockList[b].name;
    ...
    getToolboxXml: function () {
      ...
      for (var genName in this.simpleGenerators) {
        for (
          ...
          var blockName =
            genName == "."
              ? generator.label + "__"
              : genName + "_" + generator.label + "__";
          categoriesInfos[generator.category].blocksXml.push(
      ...
      if (this.includeBlocks.standardBlocks.includeAll) {
        this.includeBlocks.standardBlocks.wholeCategories = [
          "input",
          "logic",
          "loops",
          "math",
      ...
      for (var iCategory = 0; iCategory < wholeCategories.length; iCategory++) {
        ...
        for (var iBlock = 0; iBlock < blocks.length; iBlock++) {
          categoriesInfos[categoryName].blocksXml.push(
            blocks[iBlock].blocklyXml
          );
        }
      }

      this.addBlocksAndCategories(
        this.includeBlocks.standardBlocks.singleBlocks || [],
        stdBlocks,
        categoriesInfos
      );
      ...
      if (typeof this.includeBlocks.variables !== "undefined") {
        Blockly.Variables.flyoutOptions.fixed =
          this.includeBlocks.variables.length > 0
            ? this.includeBlocks.variables
            : [];
        if (typeof this.includeBlocks.variablesOnlyBlocks !== "undefined") {
          ...
          for (
            var iBlock = 0;
            iBlock < this.includeBlocks.variablesOnlyBlocks.length;
            iBlock++
          ) {
            Blockly.Variables.flyoutOptions.includedBlocks[
              this.includeBlocks.variablesOnlyBlocks[iBlock]
            ] = true;
          }
        }
      ...
      for (var categoryName in categoriesInfos) {
        ...
        if (this.includeBlocks.groupByCategory) {
          ...
          xmlString +=
            "<category " +
            " name='" +
            this.strings.categories[categoryName] +
            "'" +
            " colour='" +
            colour +
            "'" +
    ...
    addExtraBlocks: function () {
      ...
      Blockly.JavaScript["controls_untilWhile"] =
        Blockly.JavaScript["controls_whileUntil"];
      Blockly.Python["controls_untilWhile"] =
        Blockly.Python["controls_whileUntil"];
      ...
      Blockly.Python["math_extra_double"] = function (block) {
        ...
        var operator = block.getFieldValue("OP");
        var arg1 =
          Blockly.Python.valueToCode(block, "A", Blockly.Python.ORDER_NONE) ||
          "0";
        var arg2 =
              ...
              args0: [
                {
                  ...
                  src:
                    ...
                    "icons/event_whenflagclicked.svg",
                  width: 24,
                  height: 24,
                  alt: "flag",
                  flip_rtl: true,
    ...
    fixScratch: function () {
      ...
      for (
        var iBlock = 0;
        iBlock < this.includeBlocks.standardBlocks.singleBlocks.length;
        iBlock++
      ) {
        ...
        if (blocklyToScratch.singleBlocks[blockName]) {
          for (
            var b = 0;
            b < blocklyToScratch.singleBlocks[blockName].length;
            b++
          ) {
  ...
  function init(
    ...
    runner.initInterpreter = function (interpreter, scope) {
      for (var objectName in context.customBlocks) {
        for (var iCategory in context.customBlocks[objectName]) {
          for (var iBlock in context.customBlocks[objectName][iCategory]
            .blocks) {
            var blockInfo =
              context.customBlocks[objectName][iCategory].blocks[iBlock];
            var code = context.strings.code[blockInfo.name];

            if (typeof code == "undefined") code = blockInfo.name;

            interpreter.setProperty(
              scope,
              code,
              interpreter.createAsyncFunction(blockInfo.handler)
            );
          }
        }
      }
      ...
      interpreter.setProperty(
        scope,
        "program_end",
        interpreter.createAsyncFunction(context.program_end)
      );
    ...
    runner.initCodes = function (codes) {
      ...
      for (var iInterpreter = 0; iInterpreter < codes.length; iInterpreter++) {
        ...
        interpreters.push(
          new Interpreter(codes[iInterpreter], runner.initInterpreter)
        );
...
function testLevelSpecific() {
  ...
  for (var iTest = 0; iTest < tests.length; iTest++) {
    ...
    if (JSON.stringify(res) != JSON.stringify(tests[iTest].out)) {
      // TODO better way to compare two objects
      console.error(
        "Test " + iTest + " failed: returned " + JSON.stringify(res)
      );
    }
  }
}
...
var initBlocklySubTask = function (subTask) {
  ...
  subTask.loadLevel = function (curLevel) {
    ...
    this.blocklyHelper.includeBlocks = extractLevelSpecific(
      this.context.infos.includeBlocks,
      curLevel
    );

    this.blocklyHelper.load(
      stringsLanguage,
      this.display,
      this.data[curLevel].length
    );
  ...
  var initContextForLevel = function (iTestCase) {
    ...
    var prefix =
      "Test " + (subTask.iTestCase + 1) + "/" + subTask.nbTestCases + " : ";
  ...
  subTask.changeSpeed = function (speed) {
    ...
    if (
      ...
      this.context.runner.nbRunning() == 0
    ) {
      this.run();
