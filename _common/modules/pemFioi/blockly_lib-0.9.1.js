...
var localLanguageStrings = {
  ...
  de: {
    ...
    limitBlocksOver:
      "Noch {remainingBlocks} Bausteine zusätzlich zum Limit von {maxBlocks} verfügbar.", // TODO :: stimmt das?
    previousTestcase: " < ",
    nextTestcase: " > ",
  ...
  return {
    ...
    loadHtml: function(nbTestCases) {
      ...
      var gridButtonsAfter =
        ...
        "' onclick='task.displayedSubTask.stop()'/><br/><br/>";

      if (nbTestCases > 1) {
        gridButtonsAfter +=
          "<input type='button' value='" +
          this.strings.runProgram +
          "' onclick='task.displayedSubTask.run()'/>&nbsp;&nbsp;";
      }
      gridButtonsAfter +=
        "<button onclick='task.displayedSubTask.submit()'>" +
        (this.scratchMode
          ? "<img src='" +
            this.mediaUrl +
    ...
    getToolboxXml: function() {
      ...
      for (var iCategory = 0; iCategory < wholeCategories.length; iCategory++) {
        ...
        if (!(blocks instanceof Array)) {
          // just for now, maintain backwards compatibility
    ...
    addExtraBlocks: function() {
      ...
      if (this.scratchMode) {
        ...
        Blockly.Blocks["robot_start"] = {
          init: function() {
            this.appendDummyInput().appendField(that.strings.startingBlockName);
            this.setNextStatement(true);
            this.setColour(210);
            this.setTooltip("");
