(function() {
  ...
  function getLanguageString(key) {
    ...
    var ls = languageStrings[window.stringsLanguage]
      ? languageStrings[window.stringsLanguage]
      : languageStrings["en"];
    var str = ls[key];
    return str ? str : "";
  }
  ...
  var miniPlatformValidate = function(task) {
    return function(mode, success, error) {
      ...
      if (
        ...
        platform.registered_objects.length > 0
      ) {
        platform.trigger("validate", [mode]);
      } else {
        // Try to validate
        task.getAnswer(function(answer) {
          task.gradeAnswer(answer, task_token.getAnswerToken(answer), function(
            score,
            message
          ) {
            if (success) {
      ...
      $("head").append(
        '<link rel="stylesheet"type="text/css" href="' +
          (window.modulesPath
            ? window.modulesPath
            : "../../../_common/modules") +
          '/integrationAPI.01/official/miniPlatform.css">'
      );
