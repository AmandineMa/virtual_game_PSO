  ...
  if (!isCrossDomain()) {
    ...
    platform = {
      ...
      updateHeight: function(height, success, error) {
        return platform.parent_platform.updateDisplay(
          { height: height },
          success,
          error
        );
      ...
      getTaskParams: function(key, defaultValue, success, error) {
        return platform.parent_platform.getTaskParams(
          key,
          defaultValue,
          success,
          error
    ...
    platform.initWithTask = function(task) {
      ...
      chan.bind("task.load", function(trans, views) {
        task.load(
          views,
          callAndTrigger(trans.complete, "load", trans.error, [views]),
          trans.error
        );
        trans.delayReturn(true);
      });
      chan.bind("task.unload", function(trans) {
        task.unload(
          callAndTrigger(trans.complete, "unload", trans.error, null),
          trans.error
        );
        trans.delayReturn(true);
      });
      chan.bind("task.getHeight", function(trans) {
        task.getHeight(trans.complete, trans.error);
        trans.delayReturn(true);
      });
      chan.bind("task.getMetaData", function(trans) {
        task.getMetaData(trans.complete, trans.error);
        trans.delayReturn(true);
      });
      chan.bind("task.getViews", function(trans) {
        task.getViews(trans.complete, trans.error);
        trans.delayReturn(true);
      });
      chan.bind("task.showViews", function(trans, views) {
        task.showViews(
          views,
          callAndTrigger(trans.complete, "showViews", trans.error, [views]),
          trans.error
        );
        trans.delayReturn(true);
      });
      chan.bind("task.updateToken", function(trans, token) {
        task.updateToken(token, trans.complete, trans.error);
        trans.delayReturn(true);
      });
      chan.bind("task.reloadAnswer", function(trans, answer) {
        task.reloadAnswer(
          answer,
          callAndTrigger(trans.complete, "reloadAnswer", trans.error, [answer]),
          trans.error
        );
        trans.delayReturn(true);
      });
      chan.bind("task.getAnswer", function(trans) {
        task.getAnswer(trans.complete, trans.error);
        trans.delayReturn(true);
      });
      chan.bind("task.getState", function(trans) {
        task.getState(trans.complete, trans.error);
        trans.delayReturn(true);
      });
      chan.bind("task.getResources", function(trans) {
        task.getResources(trans.complete, trans.error);
        trans.delayReturn(true);
      });
      chan.bind("task.reloadState", function(trans, state) {
        task.reloadState(
          state,
          callAndTrigger(trans.complete, "reloadState", trans.error, [state]),
          trans.error
        );
        trans.delayReturn(true);
      });
      chan.bind("grader.gradeTask", function(trans, params) {
        gradeAnswer(params, trans.complete, trans.error);
        trans.delayReturn(true);
      });
      chan.bind("task.gradeAnswer", function(trans, params) {
        gradeAnswer(params, trans.complete, trans.error);
        trans.delayReturn(true);
      });
    };
...
(function() {
  ...
  window.displayHelper = {
    ...
    formatTranslation: function(s, args) {
      return s.replace(/\{([^}]+)\}/g, function(_, match) {
        return args[match];
      });
    },
    ...
    languageStrings: {
      fr: {
        ...
        levelVersionName_hard: "difficult version",
        levelVersionName_easy_stars: "2-star version",
        levelVersionName_medium_stars: "3-star version",
        levelVersionName_hard_stars: "4-star version",
        levelName_easy: "Easy",
        levelName_medium: "Medium",
        levelName_hard: "Difficult",
        warningTimeout:
          "<p> Be careful, you've been on this issue for over {0} minutes. </p> <p> You should probably change the subject, by clicking on the button at the top right. </p> > ",
        alright: "Okay",
        moveOn: "Move on",
        solvedMoveOn:
          "You have fully resolved this question, move on to another question.",
        confirmRestart: "Are you sure you want to start this version again?",
        yes: "Yes",
        no: "No",
        tryHardLevel: "We suggest you try the 4-star version.",
        tryMediumLevel: "We suggest you try the 3-star version.",
        tryNextTask:
          "We suggest you move on to the next topic. If you have time, come back and try the next version later.",
        yourScoreIsNow: "Your score is now:",
        worseScoreStays: "It's worse than before; your score remains:",
        scoreStays: "Your score remains the same:",
        score: "Score:",
        noPointsForLevel: "You don't have any points yet on this version.",
        outOf: "on",
        tryToDoBetterOrChangeTask:
          "Try to do even better, or go to another question.",
        tryToDoBetterOrMoveToNextLevel:
          "Try to do even better, or upgrade to a more difficult version.",
        bestPossibleScoreCongrats:
          "This is the best possible score on this subject; congratulations!",
        forMorePointsMoveToNextLevel:
          "To get more points, upgrade to a more difficult version.",
        youDidBetterBefore: "You had done better before.",
        scoreStays2: "Your score remains the same.",
        reloadBestAnswer: "Reload your best answer.",
        validate: "Validate",
        restart: "Restart",
        harderLevelSolved:
          "Please note: you have already resolved a more difficult version. You will not be able to earn additional points with this version.",
        showLevelAnyway: "See it anyway",
        scoreObtained: "Score obtained:",
        hardVersionTakesTime:
          "Resolving a {0} can take a long time; consider answering the questions in {1} first to earn points quickly.",
        illKeepThatInMind: "I'll take care",
        harderLevelAvailable:
          "Note that for this question, you can directly resolve a more difficult version than this.",
        lockedLevel:
          "This version is locked. Solve the previous one to see it!",
        gradeThisAnswer: "Evaluate this answer", // The following messages are used for tasks with no feedback

        saveAnswer: "Save your answer",
        answerSavedModifyOrCancelIt:
          "Your response has been saved. You can edit it, or {0} and try again.",
        cancelIt: "cancel",
        warningDifferentAnswerSaved:
          "Please note: a different answer is saved.",
        youMay: "You can {0}.",
        reloadIt: "reload",
        ...
        point: "point",
        points: "points", // The following messages are used when viewing tasks after contest is over
        contestOverScoreStays:
          "The contest is over, your answer is not saved and your score remains:",
        scoreWouldBecome: "With this answer, your score would be:",
        reloadValidAnswer: "Reload the validated answer.",
        contestOverAnswerNotSaved:
          "The competition is over: your answer is not saved.",
        scoreWouldStay: "With this answer, your score would remain the same:",
        answerNotSavedContestOver:
          "The contest is over, your answer has not been saved. You can {0}.",
        reloadSubmittedAnswer: "reload the response you submitted",
        difficultyWarning:
          "<strong> Please note: </strong> it takes a long time to resolve this version. <br/> You will be able to resolve 2 and 3 star versions of other subjects much faster.",
        enemyWarning:
          "<strong> Warning: </strong> in this challenge, the computer will prevent you from finding the solution by chance."
      },
      en: {
        version: "Version",
        levelVersionName_easy: "easy version",
        ...
        levelName_hard: "Hard",
        warningTimeout:
          "<p>Warning, it has been more than {0} minutes since you started working on this task.</p><p>You should probably switch to a diffrent task, by clicking on the button on the top-right.</p>",
        alright: "Alright",
        moveOn: "Move on",
        solvedMoveOn:
          "You solved this task completely, move on to another task.",
        ...
        tryNextTask:
          "We suggest you try the next task. If you still have time, come back later and try the next version of this task.",
        yourScoreIsNow: "Your score is now:",
        worseScoreStays: "This is not as good as before. Your score stays:",
        scoreStays: "Your score stays the same:",
        score: "Score:",
        noPointsForLevel:
          "You have not received any points yet on this version.",
        outOf: " out of ",
        tryToDoBetterOrChangeTask:
          "Try to do even better, or move on to another task.",
        tryToDoBetterOrMoveToNextLevel:
          "Try to do even better, or move on to a more difficult version.",
        bestPossibleScoreCongrats:
          "This is the best possible score on this task, congratulations!",
        forMorePointsMoveToNextLevel:
          "To obtain more points, move on to a harder version of this task.",
        ...
        restart: "Restart",
        harderLevelSolved:
          "Warning: you already solved a harder version of this task. You won't be able to obtain extra points with this version.",
        showLevelAnyway: "Show it to me anyways.",
        scoreObtained: "Obtained score:",
        hardVersionTakesTime:
          "Solving a {0} can take a lot of time. Consider working on the {1} to gain points quickly.",
        illKeepThatInMind: "I'll consider it.",
        harderLevelAvailable:
          "Note that for this task, you may try to directly work on a harder version than this one.",
        lockedLevel:
          "This version is locked. Solve the previous version to display it!",
        gradeThisAnswer: "Grade this answer",

        // The following messages are used for tasks with no feedback
        saveAnswer: "Save this answer",
        answerSavedModifyOrCancelIt:
          "Your answer has been saved. You can modify it, or {0} and restart.",
        cancelIt: "cancel it",
        warningDifferentAnswerSaved:
          "Warning: a different answer was saved before.",
        youMay: "You may {0}.",
        reloadIt: "reload it",
        saveThisNewAnswer: "Save this new answer",
        ...
        contestOverScoreStays:
          "The contest being over, your new answer was not saved and your score stays:",
        scoreWouldBecome: "With this answer, your score would be:",
        reloadValidAnswer: "Reload the validated answer.",
        contestOverAnswerNotSaved:
          "The contest being over, your new answer was not saved.",
        scoreWouldStay: "With this answer, your score would stay the same:",
        answerNotSavedContestOver:
          "The contest being over, your answer was not saved. You may {0}.",
        reloadSubmittedAnswer: "reload the validated answer",
        difficultyWarning:
          "<strong>Warning:</strong> solving this version takes time.<br/>You would solve the 2 or 3 star versions of other tasks more quickly.",
        enemyWarning:
          "<strong>Warning:</strong> in this challenge, the computer will make sure you don't find the solution by chance."
      },
      sv: {
        version: "Version",
        ...
        levelName_hard: "Svår",
        warningTimeout:
          "<p>Varning: det har gått mer än {0} minuter sedan du började med den här uppgiften. </p><p>Du borde kanske byta till en annan uppgift, genom att klicka på knappen uppe till höger.</p>",
        alright: "Okej",
        moveOn: "Gå vidare",
        solvedMoveOn:
          "Du löste uppgiften helt! Gå nu vidare till en annan uppgift.",
        confirmRestart:
          "Är du säker på att du vill börja om med den här versionen?",
        yes: "Ja",
        no: "Nej",
        ...
        tryNextTask:
          "Vi föreslår att du provar nästa uppgift. Kom tillbaka senare och prova en svårare version av den här uppgiften.",
        yourScoreIsNow: "Din poäng är nu:",
        worseScoreStays:
          "Det är inte lika bra som tidigare. Poängen fortfarande:",
        scoreStays: "Din poäng är fortfarande:",
        score: "Poäng:",
        noPointsForLevel:
          "Du har inte fått några poäng än på den här versionen.",
        outOf: " utav ",
        tryToDoBetterOrChangeTask:
          "Försök klara det ännu bättre, eller gå vidare till en annan uppgift.",
        tryToDoBetterOrMoveToNextLevel:
          "Försök klara det ännu bättre, eller gå vidare till en svårare version.",
        bestPossibleScoreCongrats:
          "Detta är högsta möjliga poäng på den här uppgiften. Grattis!",
        forMorePointsMoveToNextLevel:
          "För att få mer poäng, gå vidare till en svårare version av den här uppgiften.",
        ...
        restart: "Börja om",
        harderLevelSolved:
          "Varning: du har redan löst en svårare version av den här uppgiften. Du kommer inte kunna få mer poäng med den här versionen.",
        showLevelAnyway: "Visa den ändå.",
        scoreObtained: "Uppnådd poäng:",
        hardVersionTakesTime:
          "Att lösa en {0} kan ta lång tid. Fundera på om du ska jobba med en {1} för att tjäna poäng snabbare.",
        illKeepThatInMind: "Jag ska tänka på det.",
        harderLevelAvailable:
          "Notera att på den här uppgiften kan du direkt försöka med en svårare version än denna.",
        lockedLevel:
          "Den här versionen är låst. Lös den föregående nivån för att visa den!",
        gradeThisAnswer: "Bedöm svaret",

        // The following messages are used for tasks with no feedback
        saveAnswer: "Spara svaret",
        answerSavedModifyOrCancelIt:
          "Ditt svar har sparats. Du kan ändra det, eller {0} och börja om.",
        cancelIt: "avbryta det",
        warningDifferentAnswerSaved:
          "Varning: ett annat svar finns redan sparat.",
        youMay: "Du kan {0}.",
        ...
        // The following messages are used when viewing tasks after contest is over
        contestOverScoreStays:
          "Eftersom tävlingen är över sparas inte ditt svar och din poäng ändras inte.",
        scoreWouldBecome: "Med det här svaret, skulle din poäng vara:",
        reloadValidAnswer: "Ladda in det kontrollerade svaret.",
        contestOverAnswerNotSaved:
          "Eftersom tävlingen är över sparas inte ditt svar.",
        scoreWouldStay: "Med det här svaret, skulle din poäng inte ändras:",
        answerNotSavedContestOver:
          "Eftersom tävlingen är över sparas inte ditt svar. Du kan {0}.",
        reloadSubmittedAnswer: "ladda in det kontrollerade svaret på nytt",
        difficultyWarning:
          "<strong>Varning:</strong> att lösa den här versionen tar lång tid.<br/>Det kan gå snabbare att lösa 2- eller 3-stjärniga versioner av andra uppgifter.",
        enemyWarning:
          "<strong>Varning:</strong> i den här utmaningen kommer datorn se till att du inte hittar lösningen av en slump."
      },
      fi: {
        version: "Versio",
        ...
        warningTimeout:
          "<p>Varoitus: on kulunut jo yli {0} minuuttia siitä, kun aloit tekemään tätä tehtävää.</p><p>Sinun mahdollisesti kannattaisi siirtyä yrittämään jotain toista tehtävää, klikkaamalla oikean yläkulman nappia.</p>",
        alright: "Ok",
        moveOn: "Siirry eteenpäin",
        solvedMoveOn:
          "Ratkaisit tämän tehtävän kokonaan, siirry nyt seuraavaan tehtävään.",
        confirmRestart:
          "Oletko varma, että haluat aloittaa tämän version alusta?",
        yes: "Kyllä",
        no: "Ei",
        tryHardLevel: "Ehdotamme, että kokeilet 4 tähden versiota.",
        tryMediumLevel: "Ehdotamme, että kokeilet 3 tähden versiota.",
        tryNextTask:
          "Ehdotamme, että kokeilet seuraavaa tehtävää. Jos sinulle jää vielä aikaa, voit myöhemmin palata takaisin tämän tehtävän pariin.",
        ...
        outOf: " / ",
        tryToDoBetterOrChangeTask:
          "Yritä saada vielä paremmat pisteet, tai siirry toiseen tehtävään.",
        tryToDoBetterOrMoveToNextLevel:
          "Yritä saada vielä paremmat pisteet, tai siirry saman tehtävän vaikeampaan versioon.",
        bestPossibleScoreCongrats:
          "Onnittelut: saavutit tehtävän maksimipistemäärän!",
        forMorePointsMoveToNextLevel:
          "Siirry tehtävän vaikeampaan versioon saadaksesi enemmän pisteitä.",
        ...
        harderLevelSolved:
          "Varoitus: olet jo ratkaissut vaikeamman version tästä tehtävästä. Tämän helpomman version ratkaiseminen ei voi korottaa pistemäärääsi.",
        showLevelAnyway: "Siirry joka tapauksessa.",
        scoreObtained: "Saatu pistemäärä:",
        hardVersionTakesTime:
          "{0} voi viedä runsaasti aikaa. {1} voi tuottaa pisteitä nopeammin.",
        illKeepThatInMind: "Huomioin tämän.",
        harderLevelAvailable:
          "Huomaa, että voit myös suoraan koittaa ratkaista vaikeampaa versiota tästä tehtävästä.",
        lockedLevel:
          "Tämä versio on vielä lukittu: ratkaise ensin helpompi versio!",
        gradeThisAnswer: "Pisteytä tämä vastaus",

        // The following messages are used for tasks with no feedback
        saveAnswer: "Tallenna vastaus",
        answerSavedModifyOrCancelIt:
          "Vastauksesi on tallennettu. Voit muokata sitä, tai {0} ja aloittaa uudelleen alusta.",
        cancelIt: "perua sen",
        warningDifferentAnswerSaved:
          "Varoitus: toisenlainen vastaus on tallennettu jo aiemmin.",
        youMay: "Voit {0}.",
        ...
        // The following messages are used when viewing tasks after contest is over
        contestOverScoreStays:
          "Kilpailu on jo päättynyt, joten uutta vastaustasi ei enää tallennettu ja pistemääränäsi säilyy:",
        scoreWouldBecome:
          "Jos tämäkin vastaus huomioitaisiin, pistemääräsi olisi:",
        reloadValidAnswer: "Palauta aiemmin hyväksytty vastaus.",
        contestOverAnswerNotSaved:
          "Kilpailu on jo päättynyt, joten uutta vastaustasi ei enää tallennettu.",
        scoreWouldStay:
          "Jos tämäkin vastaus huomioitaisiin, pistemääräsi olisi yhä:",
        answerNotSavedContestOver:
          "Kilpailu on jo päättynyt, joten uutta vastaustasi ei enää tallennettu. Voit {0}.",
        reloadSubmittedAnswer: "palauttaa aiemmin lähetetyn vastauksen",
        difficultyWarning:
          "<strong>Varoitus:</strong> tämän version ratkaiseminen vie aikaa.<br/>Saat luultavasti ratkaistua 2 tai 3 tähden version nopeammin.",
        enemyWarning:
          "<strong>Varoitus:</strong> tässä tehtävässä tietokone pyrkii varmistamaan, ettet voi löytää ratkaisua sattumalta."
      },
      de: {
        version: "Version",
        ...
        warningTimeout:
          "<p>Achtung, du bist schon seit {0} Minuten bei dieser Frage.</p><p>Du solltest jetzt zu einer anderen Aufgabe wechseln.</p>",
        alright: "OK",
        moveOn: "Fortfahren",
        solvedMoveOn:
          "Du hast diese Frage bereits vollständig gelöst. Wähle eine andere Frage zum Bearbeiten aus.",
        confirmRestart:
          "Bist du sicher, dass du diese Version neustarten möchtest?",
        yes: "Ja",
        no: "Nein",
        tryHardLevel:
          "Wir schlagen dir vor, die 4-Sterne-Version zu bearbeiten.",
        tryMediumLevel:
          "Wir schlagen dir vor, die 3-Sterne-Version zu bearbeiten",
        tryNextTask:
          "Wir schlagen dir vor, die nächste Aufgabe zu bearbeiten. Wenn du am Ende noch Zeit hast, kannst du hierher zurückkehren und die schwerere Version bearbeiten.",
        ...
        score: "Punktestand:",
        noPointsForLevel:
          "Du hast noch keine Punkte für diese Version erhalten.",
        outOf: " von ",
        tryToDoBetterOrChangeTask:
          "Versuche, dich zu verbessern oder wähle eine andere Frage.",
        tryToDoBetterOrMoveToNextLevel:
          "Versuche, dich zu verbessern oder wähle schwierigere Version.",
        bestPossibleScoreCongrats:
          "Das ist die bestmögliche Punktzahl für diese Aufgabe. Glückwunsch!",
        forMorePointsMoveToNextLevel:
          "Wähle eine schwerere Version aus, um noch mehr Punkte zu bekommen.",
        ...
        restart: "Neustarten",
        harderLevelSolved:
          "Achtung: Du hast schon eine schwerere Version gelöst. Du kannst mit dieser Version keine zusätzlichen Punkte bekommen.",
        showLevelAnyway: "Trotzdem anzeigen",
        scoreObtained: "Erhaltene Punkte:",
        hardVersionTakesTime:
          "Eine {0} zu lösen kann dich viel Zeit kosten; Denke zunächst daran, die Fragen in {1} zu beantworten, um schnell Punkte zu bekommen.",
        illKeepThatInMind: "Ich hab das verstanden",
        harderLevelAvailable:
          "Beachte, dass du bei dieser Frage direkt zu einer schwereren Version gehen kannst.",
        lockedLevel:
          "Diese Version ist noch gesperrt. Löse die vorherige um diese freizuschalten.",
        gradeThisAnswer: "Diese Antwort auswerten",

        // The following messages are used for tasks with no feedback
        saveAnswer: "Antwort speichern",
        answerSavedModifyOrCancelIt:
          "Deine Antwort wurde eingereicht. Du kannst sie noch bearbeiten, oder {0} und neu beginnen.",
        cancelIt: "Abbrechen",
        warningDifferentAnswerSaved:
        ...
        // The following messages are used when viewing tasks after contest is over
        contestOverScoreStays:
          "Der Contest ist vorbei, deine Antwort wurde nicht eingereicht und deine Punktestand bleibt:",
        scoreWouldBecome: "Mit dieser Antwort wäre dein Punktestand:",
        reloadValidAnswer: "Die gültige Antwort neu laden.",
        contestOverAnswerNotSaved:
          "Der Contest ist vorbei, deine Antwort wurde nicht eingereicht.",
        scoreWouldStay: "Mit dieser Antwort bliebe dein Punktestand gleich:",
        answerNotSavedContestOver:
          "Der Contest ist vorbei, deine Antwort wurde nicht eingereicht. Du kannst {0}.",
        reloadSubmittedAnswer: "Lade die Lösung, die du eingereicht hast",
        difficultyWarning:
          "<strong>Achtung:</strong> diese Version zu lösen kann einige Zeit in Anspruch nehmen.<br/>Die 2- und 3-Stern Version von anderen Aufgaben lassen sich schneller lösen.",
        enemyWarning:
          "<strong>Attention :</strong> dans ce défi, l'ordinateur vous empêchera de trouver la solution par hasard."
      },
      ar: {
        version: "المستوى",
        ...
        levelName_hard: "صعب",
        warningTimeout:
          "<p>لقد مر وقت طويل منذ أن بدأت في هذه المسألة, من الأفضل أن تبدأ في مسألة أخرى حتى لا يضيع الوقت</p>",
        alright: "حسناً",
        moveOn: "استمر",
        ...
        tryNextTask:
          "نقترح أن تبدأ في المسألة التالية, وإذا تبقى عندك وقت يمكنك حل المستوى الأصعب في هذه المسألة لاحقاً",
        ...
        bestPossibleScoreCongrats:
          "مبروك ... لقد حصلت على أعلى درجة في هذا السؤال",
        forMorePointsMoveToNextLevel:
          "للحصول على المزيد من النقاط جاوب على المستوى الأصعب",
        ...
        harderLevelSolved:
          "لقد قمت بحل المستوى الأصعب في هذا السؤال, لن تتمكن من الحصول على درجات أعلى في هذا السؤال",
        ...
        illKeepThatInMind: "I'll consider it.",
        harderLevelAvailable:
          "تنبيه: يمكنك حل المستوى الأصعب في هذه المسألة مباشرة",
        ...
        answerSavedModifyOrCancelIt:
          "تم حفظ إجابتك, يمكنك تعديلها أو بدأها من جديد",
        cancelIt: "احذفها",
        ...
        // The following messages are used when viewing tasks after contest is over
        contestOverScoreStays:
          "المسابقة انتهت. إجابتك الجديدة لم تحفظ ومجموع نقاطك ما زال:",
        ...
        difficultyWarning:
          "تنبيه: حل هذه النسخة سوف يستغرق وقت كثير. الإفضل أن تبدأ في حل مسائل أخرى",
        enemyWarning:
          "تحذير: في هذه المسألة سيمنعك الكمبيوتر من إيجاد الحل عن طريق الصدفة. فكر جيداً"
      },
      es: {
        version: "Versión",
        ...
        warningTimeout:
          "<p>Atención, ya lleva {0} minutos en esta pregunta.</p><p>Le recomendamos cambiar de tema haciendo click sobre el botón de arriba a la derecha.</p>",
        alright: "De acuerdo",
        moveOn: "Pasar a la siguiente",
        solvedMoveOn:
          "Ha resuelto completamente esta pregunta. Pase a otra pregunta.",
        ...
        tryNextTask:
          "Nous vous proposons de passer au sujet suivant. S'il vous reste du temps, vous reviendrez plus tard essayer la version suivante.",
        yourScoreIsNow: "Su puntuación es ahora :",
        worseScoreStays:
          "Esto no está tan bien como antes; su puntuación se mantiene en :",
        scoreStays: "Su puntuación se mantiene igual :",
        score: "Puntuación :",
        noPointsForLevel: "Aún no ha recibido puntos en esta versión.",
        outOf: " de ",
        tryToDoBetterOrChangeTask:
          "Intente nuevamente para obtener una mejor puntuación, o pase a la siguiente pregunta.",
        tryToDoBetterOrMoveToNextLevel:
          "Intente nuevamente para obtener una mejor puntuación, o pase una versión más difícil.",
        bestPossibleScoreCongrats:
          "Esta es la mejor puntuación posible en este problema, ¡felicitaciones!",
        forMorePointsMoveToNextLevel:
          "Para obtener más puntos, pase a una versión más difícil.",
        ...
        restart: "Reiniciar",
        harderLevelSolved:
          "Atención: ya ha resuelto una versión más difícil. No puede ganar puntos extra con esta versión.",
        showLevelAnyway: "Mostrar el nivel de igual manera",
        scoreObtained: "Puntuación obtenida:",
        hardVersionTakesTime:
          "Resolver una {0} puede tomar mucho tiempo; le aconsejamos priorizar resolver las preguntas en {1} para ganar puntos rápidamente.",
        illKeepThatInMind: "Lo tendré en mente",
        harderLevelAvailable:
          "Note que para esta pregunta, puede resolver directamente una versión más difícil que esta.",
        lockedLevel:
          "Esta versión está bloqueada. Resuelva la version anterior para verla.",
        gradeThisAnswer: "Evaluar esta respuesta",

        // The following messages are used for tasks with no feedback
        saveAnswer: "Guardar su respuesta",
        answerSavedModifyOrCancelIt:
          "Su respuesta fue guardada. Puede modificarla, o bien {0} y reiniciar.",
        cancelIt: "cancelarla",
        warningDifferentAnswerSaved:
          "Atención: una respuesta diferente ha sido guardada.",
        ...
        // The following messages are used when viewing tasks after contest is over
        contestOverScoreStays:
          "El concurso está terminando, su respuesta no ha sido guardada y su puntuación se mantiene en:",
        scoreWouldBecome: "Con esta respuesta, su puntuación será :",
        reloadValidAnswer: "Volver a cargar la respuesta válida.",
        contestOverAnswerNotSaved:
          "El concurso ha terminado: su respuesta no fue guardada.",
        scoreWouldStay: "Con esta respuesta, su puntuación será la misma:",
        answerNotSavedContestOver:
          "El concurso está terminando y su respuesta no ha sido guardada. Usted puede {0}.",
        reloadSubmittedAnswer: "recargar la respuesta que ha enviado",
        difficultyWarning:
          "<strong>Advertencia:</strong> resolver esta versión toma tiempo.<br/>Usted puede resolver más rápidamente las versiones de 2 y 3 estrellas de otros problemas.",
        enemyWarning:
          "<strong>Advertencia:</strong> en este desafío, la computadora se asegurará que no encuentre la respuesta por casualidad."
      },
      sl: {
        version: "Stopnja",
        ...
        warningTimeout:
          "<p>Opozorilo: Odkar rešuješ to nalogo, je minilo že več kot {0} minut.</p><p>Najbolje, da izbereš drugo nalogo, tako da klikneš gumb v zgornjem desnem kotu.</p>",
        alright: "V redu",
        moveOn: "Nadaljuj",
        ...
        tryNextTask:
          "Prdlagamo, da poizkusiš naslednjo nalogo. Če bo ostalo dovolj časa, se vrni in poizkusi naslednjo stopnjo te naloge.",
        yourScoreIsNow: "Tvoj rezultat je:",
        worseScoreStays:
          "Rešitev ni tako dobra, kot prejšnja. Tvoj rezultat ostaja:",
        scoreStays: "Tvoj rezultat ostaja enak:",
        score: "Rezultat:",
        noPointsForLevel: "Na tej stopnji nisi dobil(-a) še nobene točke.",
        outOf: " od ",
        tryToDoBetterOrChangeTask:
          "Poskusi nalogo rešiti še bolje ali se premakni na naslednjo nalogo.",
        tryToDoBetterOrMoveToNextLevel:
          "Poskusi nalogo rešiti še bolje ali se premakni na težjo stopnjo.",
        bestPossibleScoreCongrats:
          "Čestitamo, to je najboljši možni rezultat te naloge!",
        forMorePointsMoveToNextLevel:
          "Če želiš dobiti še več točk, se premakni na težjo stopnjo te naloge.",
        ...
        restart: "Začni znova",
        harderLevelSolved:
          "Opozorilo: Rešil(-a) si že težjo stopnjo te naloge. S to stopnjo ne boš dobil(-a) dodatnih točk.",
        showLevelAnyway: "Vseeno mi pokaži stopnjo.",
        scoreObtained: "Dobljeni rezultat:",
        hardVersionTakesTime:
          "Reševanje {0} lahko traja veliko časa. Razmisli o reševanju {1}, da boš hitro dobil(-a) točke.",
        illKeepThatInMind: "Razmislil(-a) bom.",
        harderLevelAvailable:
          "Ne pozabi, da lahko to nalogo rešuješ na težji stopnji kot je ta.",
        lockedLevel:
          "Ta stopnja je zaklenjena! Za pikaz moraš najprej rešiti prejšnjo stopnjo!",
        gradeThisAnswer: "Oceni ta odgovor",

        // The following messages are used for tasks with no feedback
        saveAnswer: "Shrani ta odgovor",
        answerSavedModifyOrCancelIt:
          "Tvoj odgovor je bil shranjen. Lahko ga spremeniš ali {0} in začneš znova.",
        cancelIt: "prekličeš",
        warningDifferentAnswerSaved:
          "Opozorilo: Prej je bil shranjen drugačen odgovor.",
        youMay: "Lahko ga {0}.",
        ...
        // The following messages are used when viewing tasks after contest is over
        contestOverScoreStays:
          "Ker je tekmovanja konec, tvoj novi odgovor ni bil shranjen in rezultat bo ostal:",
        scoreWouldBecome: "S to rešitvijo bi bil tvoj rezultat:",
        reloadValidAnswer: "Znova naloži preverjeno rešitev.",
        contestOverAnswerNotSaved:
          "Ker je tekmovanja konec, tvoj novi odgovor ni bil shranjen.",
        scoreWouldStay: "S to rešitvijo bi tvoj rezultat ostal enak:",
        answerNotSavedContestOver:
          "Ker je tekmovanja konec, tvoj novi odgovor ni bil shranjen. Lahko {0}.",
        reloadSubmittedAnswer: "znova naložiš preverjeno rešitev",
        difficultyWarning:
          "<strong>Opozorilo:</strong> Reševanje te stopnje lahko traja veliko časa. <br/>Hitreje lahko rešiš stopnje z 2 ali s 3 zvezdicami drugih nalog.",
        enemyWarning:
          "<strong>Opozorilo:</strong> Pri tej nalogi ti bo računalnik preprečil naključno rešitev."
      }
    },
    ...
    load: function(views) {
      ...
      var self = this;
      this.showScore =
        typeof views.grader !== "undefined" && views.grader === true;
      window.platform.getTaskParams(null, null, function(taskParams) {
        self.taskParams = taskParams;
        self.readOnly =
          self.taskParams.readonly === true ||
          self.taskParams.readOnly == "true";
        self.graderScore = +self.taskParams.noScore;
        self.savedAnswer = "";

        $("#difficultyWarning")
          .html(self.strings.difficultyWarning)
          .addClass("warningHeader");
        $("#enemyWarning")
          .html(self.strings.enemyWarning)
          .addClass("warningHeader");
        var addTaskHTML =
          '<div id="displayHelperAnswering" class="contentCentered">';
        ...
        for (
          var iPlacement = 0;
          iPlacement < placementNames.length;
          iPlacement++
        ) {
        ...
        var taskDelayWarning = function() {
          if (self.popupMessageShown) {
            ...
            self.showPopupMessage(
              self.formatTranslation(self.strings.warningTimeout, [
                self.timeoutMinutes
              ]),
              "blanket",
              self.strings.alright,
              null,
              null,
              "warning"
            );
        ...
        if (self.timeoutMinutes > 0) {
          self.taskDelayWarningTimeout = setTimeout(
            taskDelayWarning,
            self.timeoutMinutes * 60 * 1000
          );
        }
      });
    },
    unload: function() {
      if (this.taskDelayWarningTimeout) {
        this.taskDelayWarningTimeout = clearTimeout(
          this.taskDelayWarningTimeout
        );
      }
    ...
    setupLevels: function(initLevel, reloadWithCallbacks, levels) {
      ...
      if (!initLevel) {
        if (!this.taskParams) {
          ...
          window.platform.getTaskParams(null, null, function(taskParams) {
            self.taskParams = taskParams;
            initLevel = taskParams.options.difficulty
              ? taskParams.options.difficulty
              : "easy";
            self.doSetupLevels(initLevel);
          });
        } else {
          initLevel = this.taskParams.options.difficulty
            ? this.taskParams.options.difficulty
            : "easy";
    ...
    doSetupLevels: function(initLevel) {
      ...
      if (this.unlockedLevels > 1 && this.showMultiversionNotice) {
        this.showPopupMessage(
          this.strings.harderLevelAvailable,
          "blanket",
          this.strings.alright,
          function() {
            this.showMultiversionNotice = false;
          }
        );
    ...
    setupParams: function() {
      ...
      var maxScore =
        taskParams.maxScore !== undefined ? taskParams.maxScore : 40;
      this.levelsMaxScores = {};
      for (var i = 0; i < this.levels.length; i++) {
        var levelName = this.levels[i];
        var levelMaxScore =
          (maxScore * this.levelsRanks[levelName]) / this.maxStars;
        this.levelsMaxScores[levelName] = this.pointsAsStars
          ? levelMaxScore
          : Math.round(levelMaxScore);
      }
    },
    setupLevelsTabs: function() {
      ...
      for (curLevel in this.levelsRanks) {
        ...
        if (this.pointsAsStars) {
          tabsHTML +=
            '<span class="levelLabel">' +
            this.strings.version +
            '</span><span id="stars_' +
            this.levelsRanks[curLevel] +
            '"></span>';
        } else {
          tabsHTML +=
            this.strings["levelName_" + curLevel] +
            " — " +
            '<span id="tabScore_' +
            curLevel +
            '">0</span> / ' +
    ...
    setLevel: function(newLevel) {
      ...
      var afterReload = function() {
        ...
        if ($("#tab_" + newLevel).hasClass("lockedLevel")) {
          ...
          if (
            $("#tab_" + newLevel).hasClass("uselessLevel") &&
            self.levelsScores[newLevel] < self.levelsMaxScores[newLevel]
          ) {
            self.showPopupMessage(
              self.strings.harderLevelSolved,
              "tab",
              self.strings.showLevelAnyway,
              null,
              null,
              "warning"
            );
            ...
            self.showPopupMessage(
              self.formatTranslation(self.strings.hardVersionTakesTime, [
                self.strings[hardVersionKey],
                self.strings[easyVersionKey]
              ]),
              "tab",
              self.strings.illKeepThatInMind,
              function() {
    ...
    showPopupMessage: function(
      ...
      var imgPath = window.contestsRoot
        ? window.contestsRoot + "/" + window.contestFolder + "/"
        : "../../../_common/modules/img/";
      if (mode == "lock") {
        var buttonYes = "";
      } else if (mode == "input") {
        var buttonYes =
          ...
          (yesButtonText || this.strings.validate) +
          "</button>";
      } else {
        var buttonYes =
          '<button class="buttonYes">' +
      ...
      if (noButtonText != undefined) {
        buttonNo =
          '<button class="buttonNo" style="margin-left: 10px;">' +
          noButtonText +
          "</button>";
      }
      var popupHtml =
        '<div class="container">' +
        '<img class="beaver" src="' +
        imgPath +
        this.getAvatar(avatarMood) +
        '"/>' +
        '<img class="messageArrow" src="' +
        imgPath +
        'fleche-bulle.png"/>' +
        '<div class="message">' +
        message +
      ...
      if (mode == "input") {
        popupHtml +=
          '<input id="popupInput" type="text" value="' +
    ...
    showViews: function(views) {
      ...
      var self = this;
      this.checkAnswerInterval = setInterval(function() {
        self.checkAnswerChanged();
      }, 1000);
      ...
      if (mode == "cancel") {
        ...
        task.getAnswer(function(strAnswer) {
          if (!self.hasSolution) {
            ...
            if (self.hasLevels) {
              self.prevLevelsScores[self.taskLevel] =
                self.levelsScores[self.taskLevel];
            }
          }
    ...
    updateScoreDisplays: function(gradedLevel) {
      ...
      if (this.pointsAsStars) {
        ...
        drawStars(
          ...
          this.maxStars,
          24,
          this.graderScore / maxScores.hard,
          "normal"
        );
      } else {
      ...
      if (maxScores[gradedLevel] == scores[gradedLevel]) {
        ...
        if (
          unlockedLevel < this.levels.length &&
    ...
    showValidatePopup: function(gradedLevel) {
      ...
      if (
        gradedLevel == undefined &&
        this.graderScore >= this.taskParams.maxScore - 0.001
      ) {
        avatarMood = "success";
        ...
        maxScores &&
        gradedLevel !== undefined &&
        this.graderScore >= maxScores[gradedLevel] - 0.001
      ) {
        avatarMood = "success";
        buttonText = this.strings.moveOn;
        fullMessage += "<br/><br/>";
        ...
        var nextLevel =
          levelIdx !== undefined && levelIdx < this.levels.length - 1
            ? this.levels[levelIdx + 1]
            : null;
        if (nextLevel) {
          // Offer to try next task if the user solved this difficulty slowly
          var threshold = this.thresholds[gradedLevel];
          if (!threshold) {
            if (gradedLevel == "medium") {
              threshold = this.thresholdMedium;
            } else if (gradedLevel == "easy") {
              threshold = this.thresholdEasy;
            }
          }
        ...
        if (curAnswer != self.prevAnswer) {
          try {
            if (self != top && parent.Tracker) {
              var data = {
                dataType: "nonSavedAnswer",
                teamID: parent.teamID,
                questionKey: parent.currentQuestionKey,
                answer: curAnswer
              };
    ...
    getFullFeedbackSavedMessage: function(taskMode) {
      ...
      if (this.submittedAnswer !== "" && this.prevSavedScore !== undefined) {
        if (!this.hasSolution) {
          ...
          if (this.prevSavedScore != this.submittedScore) {
            scoreDiffMsg =
              this.strings.contestOverScoreStays +
              " " +
              this.prevSavedScore +
              ". " +
              this.strings.scoreWouldBecome;
      ...
      scoreDiffMsg +=
        ...
        this.graderScore +
        this.strings.outOf +
        this.taskParams.maxScore +
        ".";
      if (
        (this.hasSolution && this.savedAnswer != this.prevAnswer) ||
        (this.graderScore > 0 &&
          (taskMode == "saved_changed" || showRetrieveAnswer))
      ) {
        scoreDiffMsg +=
          ' <a href="#" onclick="displayHelper.retrieveAnswer(); return false;">' +
          this.strings.reloadValidAnswer +
          "</a>";
      }
      return scoreDiffMsg;
    },
      ...
      if (!answerExists) {
        if (this.levelsScores[this.taskLevel] > 0) {
          if (this.hideScoreDetails) {
            message =
              this.strings.scoreObtained +
              ' <span id="answerScore">' +
              this.levelsScores[this.taskLevel] +
              " " +
              strPoint +
              " " +
              this.strings.outOf +
          ...
          if (this.graderMessage !== "") {
            if (!this.hideValidateButton && !this.hasSolution) {
              return (
                ...
                strValidate +
                '" onclick="displayHelper.callValidate();" ' +
                disabledStr +
                "/>"
              );
            }
        ...
        case "unsaved_changed":
          if (!this.hideValidateButton) {
            if (this.hasSolution) {
              return (
                '<input type="button" value="' +
                this.strings.gradeThisAnswer +
                '" onclick="displayHelper.validate(\'test\');" ' +
                disabledStr +
                "/>"
              );
            } else {
    ...
    updateMessages: function() {
      ...
      if (
        this.submittedAnswer !== "" &&
        this.submittedAnswer != this.savedAnswer
      ) {
      ...
      if (this.showScore) {
        if (!this.hideRestartButton) {
          messages.cancel =
            ...
            '" onclick="displayHelper.restartAll();"' +
            disabledStr +
            "/></div>";
        }
        messages.graderMessage = this.getFullFeedbackGraderMessage(taskMode);
        messages.validate = this.getFullFeedbackValidateMessage(
          taskMode,
          disabledStr
        );
        if (this.hasLevels) {
        ...
        switch (taskMode) {
          ...
          case "unsaved_changed":
            if (!this.hasSolution) {
              messages.validate =
                '<input type="button" value="' +
                this.strings.saveAnswer +
                '" ' +
                "onclick=\"platform.validate('done', function(){})\" " +
                disabledStr +
                "/>";
            }
            break;
          case "saved_unchanged":
            if (!this.hasSolution) {
              messages.saved = this.formatTranslation(
                this.strings.answerSavedModifyOrCancelIt,
                [
                  "<a href='#' onclick=\"platform.validate('cancel', function(){}); return false;\" " +
                    disabledStr +
                    ">" +
                    this.strings.cancelIt +
                    "</a>"
                ]
              );
            } else {
              messages.saved = this.formatTranslation(
                this.strings.answerNotSavedContestOver,
                [
                  "<a href='#' onclick=\"displayHelper.validate('cancel'); return false;\" " +
                    disabledStr +
                    ">" +
                    this.strings.reloadSubmittedAnswer +
                    "</a>"
                ]
              );
            }
          ...
          case "saved_changed":
            messages.saved =
              ...
              this.strings.warningDifferentAnswerSaved +
              "</b> " +
              this.formatTranslation(this.strings.youMay, [
                "<a href='#' onclick='displayHelper.retrieveAnswer(); return false;'>" +
                  this.strings.reloadIt +
                  "</a>"
              ]);
            if (!this.hideValidateButton) {
              messages.validate =
                "<input type='button' value='" +
                this.strings.saveThisNewAnswer +
                "' onclick=\"platform.validate('done', function(){})\" " +
                disabledStr +
                "/>";
            }
      ...
      for (var type in messages) {
        if (
          this.loaded &&
          (typeof this.previousMessages[type] === "undefined" ||
      ...
      if (this.pointsAsStars && $("#answerScore").length) {
        drawStars(
          "answerScore",
          this.levelsRanks[this.taskLevel],
          20,
          this.levelsScores[this.taskLevel] /
            this.levelsMaxScores[this.taskLevel],
          "normal"
        );
      }
    ...
    sendBestScore: function(callback, scores, messages) {
      ...
      callback(
        ...
        messages[bestLevel] +
          " (" +
          this.strings["levelVersionName_" + bestLevel] +
          ")"
      );
    }
  };
...
Beav.Object.eq = function eq(x, y) {
  ...
  var tx = typeof x;
  var ty = typeof y;
  if (tx != ty) throw "Beav.Object.eq incompatible types";
  if (
    tx == "boolean" ||
    tx == "number" ||
    tx == "string" ||
  ...
  if ($.isArray(x)) {
    if (!$.isArray(y)) throw "Beav.Object.eq incompatible types";
...
Beav.Array.make = function(nb, initValue) {
  var t = [];
  for (var i = 0; i < nb; i++) t[i] = initValue;
  return t;
};

Beav.Array.init = function(nb, initFct) {
  var t = [];
  for (var i = 0; i < nb; i++) t.push(initFct(i));
  return t;
};

Beav.Array.indexOf = function(t, v, eq) {
  if (eq === undefined) eq = Beav.Object.eq;
  for (var i = 0; i < t.length; i++) if (eq(t[i], v)) return i;
  return -1;
};
...
Beav.Array.filterCount = function(t, filterFct) {
  var count = 0;
  for (var i = 0; i < t.length; i++) if (filterFct(t[i], i)) count++;
  return count;
};
...
Beav.Matrix.copy = function(m) {
  return Beav.Matrix.map(m, function(v) {
    return v;
  });
};

Beav.Matrix.make = function(nbRows, nbCols, v) {
  return Beav.Matrix.init(nbRows, nbCols, function() {
    return v;
  });
};
...
Beav.Matrix3D.copy = function(m) {
  return Beav.Matrix3D.map(m, function(v) {
    return v;
  });
};

Beav.Matrix3D.make = function(nbX, nbY, nbZ, v) {
  return Beav.Matrix3D.init(nbX, nbY, nbZ, function() {
    return v;
  });
};
...
Beav.Task.scoreInterpolate = function(
  minScore,
  maxScore,
  minResult,
  maxResult,
  ...
  return Math.round(
    minScore +
      ((maxScore - minScore) * (result - minResult)) / (maxResult - minResult)
  );
};
  ...
  window.implementGetResources = function(task) {
    task.getResources = function(callback) {
      ...
      res.task =
        "task" in res
          ? res.task
          : [{ type: "html", content: $("#task").html() }];
      res.solution =
        "solution" in res
          ? res.solution
          : [{ type: "html", content: $("#solution").html() }];
      res.grader = [];
      res.task_modules = [];
      ...
      if (!res.hints) {
        ...
        $(".hint").each(function(index) {
          res.hints[res.hints.length] = [
            { type: "html", content: $(this).html() }
          ];
      ...
      $("script, style, link").each(function() {
        ...
        if ($(this).hasClass("solution") && $(this).hasClass("module")) {
          ...
          $(this).hasClass("stdButtonsAndMessages") &&
          $(this).hasClass("module")
        ) {
          curDest = res.display_modules;
        } else if ($(this).hasClass("stdButtonsAndMessages")) {
          curDest = res.display;
        } else if (
          $(this).hasClass("stdAnswerTypes") &&
          $(this).hasClass("module")
        ) {
...
(function() {
  ...
  var languageStrings = {
    ar: {
      ...
      canReadSolution:
        "You can now read the solution at the bottom of this page.",
    ...
    fr: {
      ...
      canReadSolution:
        "Vous pouvez maintenant lire la solution en bas de la page.",
    ...
    de: {
      ...
      canReadSolution:
        "Du kannst dir jetzt die Lösung unten auf der Seite anschauen.",
      gradeAnswer: "Test grader"
    },
    es: {
      task: "Problema",
      ...
      if ($("#previewScorePopup").length === 0) {
        $(
          "<div id='previewScorePopup'><div style=\"background-color:#111;opacity: 0.65;filter:alpha(opacity=65);position:absolute;z-index:10;top:0px;left:0px;width:100%;height:2000px\"></div>" +
            ...
            "<div style='padding:50px'><span id='previewScoreMessage'></span><br/><br/><input type='button' onclick='$(\"#previewScorePopup\").remove()' value='OK' /></div></div></div>"
        ).insertBefore("#solution");
      }
      $("#previewScorePopup").show();
      $("#previewScoreMessage").html(
        "<b>" +
    ...
    return {
      ...
      init: function(views) {
        if (!$("#choose-view").length)
          $(document.body).append(
            '<div id="choose-view" style="margin-top:6em"></div>'
          );
        $("#choose-view").html("");
        ...
        $("#grade").remove();
        var btnGradeAnswer = $(
          '<center id="grade"><button class="btn btn-default">' +
            languageStrings[window.stringsLanguage].gradeAnswer +
            "</button></center>"
        );
          ...
          task.getAnswer(
            function(answer) {
              answer_token.get(answer, function(answer_token) {
                task.gradeAnswer(answer, answer_token, function(
                  score,
                  message,
                  scoreToken
                ) {
      ...
      reinit: function(views) {
        ...
        for (var viewName in this.lastShownViews) {
          if (!this.lastShownViews[viewName]) {
            continue;
          }
  ...
  $(document).ready(function() {
    ...
    try {
      hasPlatform =
        inIframe() &&
        typeof parent.TaskProxyManager !== "undefined" &&
        (typeof parent.generating == "undefined" || parent.generating === true);
      ...
      var platformLoad = function(task) {
        ...
        task.load(
          ...
          function() {
            ...
            if ($("#solution").length) {
              $("#task").append(
                "<center id='showSolutionButton'><button type='button' class='btn btn-default' onclick='miniPlatformShowSolution()'>" +
                  languageStrings[window.stringsLanguage].showSolution +
                  "</button></center>"
              );
            }
