  ...
  return {
    ...
    languageStrings: {
      ...
      en: {
        ...
        avoidReloadingOtherTask:
          "Warning: do not reload code for another task!",
    ...
    loadHtml: function(nbTestCases) {
      ...
      if (maxBlocks != undefined) {
        strMaxBlocks =
          "<span class='blocklyCapacity' style='display:inline-block;width:24px;text-align:right'>XXX</span>" +
          this.strings.limitBlocks1 +
          maxBlocks +
          this.strings.limitBlocks2;
      }
      $("#blocklyLibContent").html(
        "         <xml id='toolbox' style='display: none'>" +
          ...
          " <input type='file' id='input' onchange='task.displayedSubTask.blocklyHelper.handleFiles(this.files);resetFormElement($(\"#input\"))'></p>" +
          "            <p><input type='button' value='" +
          this.strings.saveProgram +
          "' onclick='task.displayedSubTask.blocklyHelper.saveProgram()' /><span id='saveUrl'></span>" +
          "         </div>"
      );
      ...
      gridButtonsAfter +=
        ...
        "' onclick='task.displayedSubTask.submit()' /><br/>" +
    ...
    load: function(language, display, nbTestCases, options) {
      ...
      for (
        ...
        if (!options.noRobot) {
          var xml =
            '<xml><block type="robot_start" deletable="false" movable="false"></block></xml>';
    ...
    initXML: function() {
      ...
      for (var iCategory = 0; iCategory < categories.length; iCategory++) {
        ...
        for (var objectName in this.generators) {
          for (
            var iGen = 0;
            iGen < this.generators[objectName].length;
            iGen++
          ) {
    ...
    getCode: function(language, codeWorkspace) {
      ...
      for (var b = 0; b < blocks.length; b++) {
        ...
        if (
          ["procedures_defnoreturn", "procedures_defreturn"].indexOf(
            block.type
          ) > -1
        ) {
          ...
          if (code[0] == "<") {
            try {
              ...
              if (window.quickAlgoInterface) {
                window.quickAlgoInterface.displayError(
                  that.strings.invalidContent
                );
              } else {
    ...
    getToolboxXml: function() {
      ...
      for (var objectName in this.generators) {
        for (var iGen = 0; iGen < this.generators[objectName].length; iGen++) {
          ...
          var blockName =
            objectName == "."
              ? generator.labelEn + "__"
              : objectName + "_" + generator.labelEn + "__";
      ...
      for (var category in blocksByCategory) {
        if (this.groupByCategory) {
          xml +=
            "<category name='" + this.strings[category] + "' colour='210'>";
        }
      ...
      if (
        ...
        if (this.groupByCategory) {
          xml +=
            "<category name='" + this.strings.variables + "' colour='330'>";
        }

        // block for each availableVariable
        for (var iVar = 0; iVar < this.availableVariables.length; iVar++) {
          xml +=
            "<block type='variables_get' editable='false'><field name='VAR'>" +
            this.availableVariables[iVar] +
            "</field></block>";
        }
        ...
        for (var iVar = 0; iVar < this.availableVariables.length; iVar++) {
          xml +=
            "<block type='variables_set' editable='false'><field name='VAR'>" +
            this.availableVariables[iVar] +
            "</field></block>";
        }
    ...
    addExtraBlocks: function() {
      ...
      Blockly.Blocks["controls_if_else"] = {
        init: function() {
          ...
          this.appendDummyInput().appendField(that.strings.do);
          this.appendStatementInput("DO");
          this.appendDummyInput().appendField(that.strings.else);
      ...
      Blockly.Python["controls_if_else"] = function(block) {
        var condition =
          Blockly.Python.valueToCode(block, "IF", Blockly.Python.ORDER_NONE) ||
          "false";
  ...
  function init(
    ...
    runner.initInterpreter = function(interpreter, scope) {
      for (var objectName in context.generators) {
        for (
          var iGen = 0;
          iGen < context.generators[objectName].length;
          iGen++
        ) {
          var generator = context.generators[objectName][iGen];
          interpreter.setProperty(
            scope,
            objectName + "_" + generator.labelEn,
            interpreter.createAsyncFunction(generator.fct)
          );
        }
      }
      interpreter.setProperty(
...
var initBlocklySubTask = function(subTask) {
  ...
  subTask.loadLevel = function(curLevel) {
    ...
    if (this.display) {
      ...
      gridHtml +=
        "<div id='grid' style='width:400px;height:200px;padding:10px'></div>";
  ...
  subTask.getDefaultAnswerObject = function() {
    var defaultBlockly =
      '<xml xmlns="http://www.w3.org/1999/xhtml"><block type="robot_start" deletable="false" movable="false" x="0" y="0"></block><block type="robot_start" deletable="false" movable="false" x="0" y="0"></block></xml>';
  ...
  subTask.getGrade = function(callback) {
    ...
    initBlocklyRunner(subTask.context, function(message, success) {
      subTask.testCaseResults[
        subTask.iTestCase
      ] = subTask.gridInfos.computeGrade(subTask.context, message);
      subTask.iTestCase++;
