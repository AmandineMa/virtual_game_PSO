...
class TfmServer:
   ...
   def updateLocalTasks(taskDirs):#DONE
      ...
      allTasks = TfmServer.getSVNTasks() + [TfmServer.svnPathCommon]
   ...
   def analyse():
      ...
      svnTasks = TfmServer.getSVNTasks(True)
      localTasks = TfmServer.getLocalTasks(True)
      svnTasksFull = TfmServer.getSVNTasks()
      ...
      for task, paths in TfmServer.findDuplicatesSVNTaskDirs(svnTasksFull):
         print("* {} :\n{}".format(task, "\n".join(paths)))
   ...
   def getLocalTasks(basename = False):
      ...
      return TfmServer.sortPathsList(tasks, basename)
      ...
      def search(path, prof=0):
         for file in sorted(TfmServer.execSVN("list", path).split("\n")):
            ...
            if file[0] in ["_", "."] or file[-1] != "/":
               continue
            if file[0].isupper() or file[:-1].isdigit():
               search(dirName, prof+1)
            else:
               allTasks.append({
                  'root':svnRoot,
                  'path':dirName.replace(svnRoot, ''),
                  'full':dirName
               })
   ...
   """
   def findDuplicatesSVNTaskDirs(listPaths):
      nameAndPaths = list(map(lambda p:[myBasename(p),p], listPaths))
      nameAndPaths.sort()
      dic = {}
      for taskPath in TfmServer.getSVNTasks():
         name = myBasename(taskPath)
         if not name in dic:
            dic[name] = []
         dic[name].append(taskPath)
